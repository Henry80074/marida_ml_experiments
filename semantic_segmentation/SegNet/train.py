...
logging.basicConfig(filename=os.path.join(root_path, 'logs','log_SegNet.log'), filemode='a',level=logging.INFO, format='%(name)s - %(levelname)s - %(message)s')
logging.info('*'*10)
...
def main(options):
    ...
    model = SegNet(input_channels = options['input_channels'],
...
if __name__ == "__main__":
    ...
    parser.add_argument('--batch', default=5, type=int, help='Batch size') # THIS BATCH SIZE HAS BEEN CHANGED FROM 5
    ...
    parser.add_argument('--checkpoint_path', default='/home/henry/Documents/marine-debris/semantic_segmentation/SegNet/trained_models', help='folder to save checkpoints into (empty = this folder)')
